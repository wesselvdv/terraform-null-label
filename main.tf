locals {
  ...
  regex_replace_chars_context            = concat(local.context_local["regex_replace_chars"], [""])
  ...
  names                                  = concat(local.context_local["name"], [""])
  ...
  name = lower(
    replace(local.name_or_context, local.regex_replace_chars, ""),
  )
  namespaces                   = concat(local.context_local["namespace"], [""])
  ...
  namespace = lower(
    replace(local.namespace_or_context, local.regex_replace_chars, ""),
  )
  environments                   = concat(local.context_local["environment"], [""])
  ...
  environment = lower(
    replace(local.environment_or_context, local.regex_replace_chars, ""),
  )
  stages                   = concat(local.context_local["stage"], [""])
  ...
  stage = lower(
    replace(local.stage_or_context, local.regex_replace_chars, ""),
  )
  delimiters                   = concat(local.context_local["delimiter"], [""])
  ...
  tags = merge(
    zipmap(
      local.context_local["tags_keys"],
      local.context_local["tags_values"],
    ),
